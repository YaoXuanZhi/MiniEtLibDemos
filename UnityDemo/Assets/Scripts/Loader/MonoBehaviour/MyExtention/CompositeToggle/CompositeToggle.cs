...
namespace Mobcast.Coffee.Toggles
{
	...
	[Serializable]
	public class ExCompositeToggleActiveData
	{
		...
		public ExCompositeToggleActiveData(GameObject go, int count)
		{
			gameObject = go;
			this.count = count;
		}
	...
	public class CompositeToggle : ParentChildRelatable<CompositeToggle>, ISerializationCallbackReceiver
	{
		...
		[SerializeField]
		private List<ExCompositeToggleActiveData> m_ExActiveDatas = new List<ExCompositeToggleActiveData>();
		
		[HideInInspector]
		public bool hasExActiveObjects;
		
		public List<ExCompositeToggleActiveData> ExActiveDatas
		{
			get
			{
		...
		public void AddExActiveDatas(ExCompositeToggleActiveData exActiveData)
		{
			m_ExActiveDatas.Add(exActiveData);
		}
		...
		public List<ExCompositeToggle> ReferenceExToggles = new List<ExCompositeToggle>();
...
//			components.Clear();
			...
			{
				ExCompositeToggleActiveData exActiveData = m_ExActiveDatas[i];
				switch (valueType)
				{
					case ValueType.Boolean:
					{
						if (indexValue < exActiveData.StateValues.Length && exActiveData.gameObject != null)
						{
							exActiveData.gameObject.SetActive(exActiveData.StateValues[indexValue] == 1);
						}
						break;
					}
					case ValueType.Index:
