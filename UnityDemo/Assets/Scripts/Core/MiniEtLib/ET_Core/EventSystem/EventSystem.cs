...
namespace ET
{
    public class EventSystem: Singleton<EventSystem>, ISingletonUpdate, ISingletonLateUpdate
    {
        ...
        public void Unload()
        {
            Queue<long> queue = this.queues[(int)InstanceQueueIndex.Unload];
            ...
            while (count-- > 0)
            {
                ...
                List<object> iUnloadSystems = this.typeSystems.GetSystems(component.GetType(), typeof (IUnloadSystem));
                ...
                foreach (IUnloadSystem iLoadSystem in iUnloadSystems)
                {
                    try
                    {
        ...
        public async ETTask PublishAsyncShare<E, T>(E entity, T a) where E : Entity where T : struct
        {
            List<EventInfo> iEvents;
            if (!this.allEvents.TryGetValue(typeof(T), out iEvents))
            {
            ...
            using (ListComponent<ETTask> list = ListComponent<ETTask>.Create())
            {
                foreach (EventInfo eventInfo in iEvents)
                {
                    ...
                    if (!(eventInfo.IEvent is AShareEvent<E, T> aEvent))
                    {
                        Log.Error($"event error: {eventInfo.IEvent.GetType().Name}");
                        continue;
                    }
        
                    list.Add(aEvent.HandleAsync(entity, a));
                }
        ...
        public void PublishShare<E, T>(E entity, T a) where E: Entity where T : struct
        {
            List<EventInfo> iEvents;
            if (!this.allEvents.TryGetValue(typeof (T), out iEvents))
            {
